...
`MergeAndPlay` は、次の iCloud 共有リンクから取得できます。
...
4. 追加後、ショートカットアプリの一覧に `MergeAndPlay` が表示されることを確認します。
...
* **追加方法**:
    ...
    3.  「`ショートカット`」の部分をタップし、ショートカットの一覧から「`XMLMergev`」を選択します。（このショートカットが存在しない場合は、エラーになります。もし連携しない場合は、このアクションを削除しても構いません。）
